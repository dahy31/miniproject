...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on("click", "#pwbtn", function(){
			...
			let pwcheckURL = "boardPwCheck.ldh";
			let reqType = "POST";
			let dataParam = {
				sbnum: $("#sbnum").val(),
				sbpw: $("#sbpw").val(),
		...
		$(document).on("click", "#U", function(){
			...
			$("#boardUpdateForm").attr({ "method":"POST"
				                        ,"action":"boardUpdate.ldh"}).submit();
		});
		//  D
		$(document).on("click", "#D", function(){
			//alert("D >>> : ");
			$("#boardUpdateForm").attr({ "method":"POST"
				                        ,"action":"boardDelete.ldh"}).submit();
		});
	});
</script>	
</head>
...
<table border=1>
	...
	<tr>
		<td class="mem">사진</td>
	<td> 
		<img src="/springLdh/fileupload/board/sm_<%= bvo.getSbfile() %>" border="1" width="40" height="50" alt="image">
	</td>
	</tr>
	</tr>
		<td class="mem">등록일</td>
...
<hr>
<!-- url붙여서 댓글 가져오기 -->
<c:import url="/rboardForm.ldh">
	<c:param name="sbnum" value="<%=bvo.getSbnum()%>"></c:param>
</c:import>

<%--
<!-- 
<jsp:include page="/rboardForm.ldh">
